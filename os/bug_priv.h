...
/* Don't use this directly, use BUG_WARN or BUG_WARN_MSG insteed */
...
#define BUG_RETURN_VAL(cond, vel) \
    do { if (cond) { __BUG_WARN_MSG((cond), 0, NULL); return (vel); } } while(0)

#define BUG_RETURN_VAL_MSG(cond, vel, ...) \
    do { if (cond) { __BUG_WARN_MSG((cond), 1, __VA_ARGS__); return (vel); } } while(0)
